...
class Renderer:
    ...
    def __init__(self, config: PluginConfig, star=None):
        ...
        self.use_html = getattr(config, "use_html_render", False)
    ...
    async def render_card(
        ...
        Args:
            result: 解析结果
            style: HTML 模板风格（仅 HTML 模式使用）

        Returns:
            图片路径或 None（需要调用方 fallback 到纯文本）
        """
        # 第一阶: HTML 渲染 (star.html_render)
        if self.use_html and self.html_renderer:
            try:
                ...
                img_path = await self.html_renderer.render_card(
                    payload, style=style
                )
                if img_path:
        ...
        try:
            ...
            logger.warning("[渲染] PIL渲染失败，降级到纯文本")
            return None
