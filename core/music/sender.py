...
class MusicSender:
    ...
    async def send_song_selection(
        self, event: AstrMessageEvent, songs: list[Song] | list[tuple[Song, str]],
        title: str | None = None, platform_name: str = "",
    ) -> None:
        """
        发送歌曲选择（单张图片展示所有歌，回退到文字）
        """
        try:
            from astrbot.api.message_components import Image as Img
            song_data = [(s, platform_name) for s in songs[:20]]
            img_bytes = await self.renderer.draw_song_list(
                songs=song_data, platform=platform_name, title=title or ""
            )
            if img_bytes and len(img_bytes) > 100:
                await event.send(event.chain_result([Img.fromBytes(img_bytes)]))
                return
        except Exception as e:
            logger.warning(f"歌曲列表渲染失败，回退文字: {e}")
    ...
    async def send_lyrics(
        ...
        """发歌词（增强版，带封面、时长、平台配色）"""
        if not song.lyrics or not song.cover_url:
    ...
    async def send_image_card(
        ...
        """发图片卡片（增强版，显示封面和热评）"""
        try:
            image_bytes = await self.renderer.draw_song_card(
                song_name=song.name or song.title or "未知歌曲",
                artist=song.artists or song.author or "未知歌手",
                duration=song.duration or 0,
