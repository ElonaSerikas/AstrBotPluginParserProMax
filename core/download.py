...
class Downloader:
    ...
    def __init__(self, config: PluginConfig):
        ...
        # 限制并发下载数
        self._download_semaphore = Semaphore(5)
    ...
    # ========== 品质选择下载 ==========
    QQ_MAX_SIZE = 1024 * 1024 * 1024  # 1GB

    async def download_video_best_quality(
        self,
        url: str,
        platform: str = "other",
        ...
        Args:
            url: 视频URL
            platform: 平台名，'qq' 时限制1GB
            max_size: 自定义大小限制，覆盖默认

        Returns:
            下载路径或 None
        """
        QUALITY_MAP = {
            "1080p_60": "bestvideo[height<=1080][fps>30]+bestaudio/best[height<=1080][fps>30]",
        ...
        limit = max_size or (
            self.QQ_MAX_SIZE if platform == "qq" else 10 * 1024 * 1024 * 1024  # 10GB
        )
        qualities = [
            "1080p_60", "1080p+", "1080p", "720p", "480p", "360p",
        ]
        for q in qualities:
            fmt = QUALITY_MAP[q]
            try:
                path = await self.ytdlp_download_video(
                    url,
                    format=fmt,
                )
    ...
    async def download_audio_best_quality(
        self,
        url: str,
    ) -> Path | None:
        """下载最高音质音频
        ...
        for fmt in audio_formats:
            try:
                path = await self.ytdlp_download_audio(
                    url, format=fmt, cookiefile=None
                )
                if path:
                    return path
            except Exception:
